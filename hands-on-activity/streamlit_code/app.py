...
with tab3:
    ...
    def _recovery_callback(lr: float, k_local: int):
        if (
            "user_latent_current" not in st.session_state
            or st.session_state.user_latent_current is None
        ):
            return

        # Build aligned movie info (ONLY movies in R_full, in correct order)
        movie_info_aligned = (
            pd.DataFrame({"movieId": all_movie_ids})
            .merge(
                movie_info_safe[["movieId", "category"]],
                on="movieId",
                how="left"
            )
            .fillna({"category": "neutral"})
        )

        neutral_mask = movie_info_aligned["category"] == "neutral"

        if neutral_mask.any():
            neutral_vec = V_scaled[neutral_mask.values, :].mean(axis=0)
        else:
            neutral_vec = V_mean

        before_vec = st.session_state.user_latent_current.copy()

        st.session_state.user_latent_current += (
            lr * (neutral_vec - st.session_state.user_latent_current)
        )

        st.session_state.user_latent_current = pad_truncate(
            st.session_state.user_latent_current,
            k_local
        )
        ...
        st.session_state.trajectory.append(
            st.session_state.user_latent_current.copy()
        )

        st.session_state["_last_recovery"] = {
            "prev_vector": before_vec,
            "title": "Neutral movie"
        }
    ...
    # ------------------------------------------------
    # Radicalization Index + Formula + Distribution
    # ------------------------------------------------
    with col1:
        ...
        # ----------------------------
        n_extreme = (top10["category"] == "extreme").sum()
        n_mild = (top10["category"] == "mildly_political").sum()
        n_total = len(top10)

        rad_idx = (n_extreme + 0.5 * n_mild) / (n_total + 1e-8)

        # Delta logic
        prev_val = st.session_state.get("prev_rad_idx", 0.0)
        delta_val = rad_idx - prev_val
        st.session_state.prev_rad_idx = rad_idx

        # ----------------------------
        # 2. Display formula + value
        # ----------------------------

        st.subheader("Radicalization Index:")
        st.markdown(
            rf"""
                $
                \frac{{\#(\text{{Extreme}}) + 0.5 \cdot \#(\text{{Mildly Political}})}}
                {{\text{{Total Recommendations}}}}
                = \mathbf{{{rad_idx * 100:.1f}\%}}
                $
                """,
                unsafe_allow_html=False
        )

        # Delta (shown separately, since LaTeX cannot embed arrows)
        st.caption(
            f"Change since last view: {delta_val * 100:+.1f}%"
        )
        st.caption(
            "Mildly political items contribute half the weight of extreme items."
        )
        ...
        cat_counts = (
            ...
            .value_counts()
            .reindex(
                ["neutral", "mildly_political", "extreme"],
                fill_value=0
            )
        )
    ...
    fig_extremeness = plot_user_political_extremeness(
